...
{
    [Info( "Rust2Discord", "sxnpxchz", "1.0.1" )]
    [SuppressMessage( "ReSharper", "UnusedMember.Local" )]
    internal class rust2discord : CovalencePlugin
    {
        #region PlayerChat
// Rust
#if RUST
        ...
        private class Request
        {
            ...
            private void Send()
            {
                ...
                _instance.webrequest.Enqueue( _url, _payload, ( code, rawResponse ) =>
                {
                    ...
                    {
                        _instance.PrintWarning( "[Rust2Discord] Не удалось! Дискорд ответил кодом: {0}. Плагин: {1}\n{2}", code, _plugin != null ? _plugin.Name : "Неизвестный плагин", response.Message );
                    }
                    else
                    {
                        try
                        ...
                        {
                            Interface.Oxide.LogException( "[Rust2Discord] Запрос обратного вызова вызвал исключение!", ex );
                        }
                    }
        ...
        private static rust2discord _instance;
        ...
        private void Loaded()
        {
            ...
            foreach ( var feature in _config.FeatureTypes )
            {
                ...
                value.Enabled = false;
                PrintWarning( $"{feature.Key} был включен, веб-перехватчик неверен." );
            }
        ...
        protected override void LoadDefaultMessages()
        {
            ...
            {
                ["ReportSyntax"] = "Ошибка синтаксиса. Пожалуйста, используйте /report \"name/id\" \"reason\"",
                ["BanSyntax"] = "Ошибка синтаксиса. Пожалуйста, используйте /ban \"name/id\" \"reason\"",
                ["MessageSyntax"] = "Ошибка синтаксиса. Пожалуйста, используйте /message \"your message\"",
                ["Multiple"] = "Найдено несколько игроков:\n{0}",
                ["BanMessage"] = "{0} был заблокирован за {1}",
                ["ReportSent"] = "Ваш отчет отправлен!",
                ["MessageSent"] = "Ваше сообщение отправлено!",
                ["NotFound"] = "Не удалось найти игрока {0}",
                ["NoReports"] = "{0} еще не сообщалось!",
                ["ReportDisallowed"] = "Вы были внесены в черный список, чтобы сообщить об игроках.",
                ["ReportAccessChanged"] = "Функция отчета для {0} сейчас {1}",
                ["ReportReset"] = "Вы сбросили счетчик отчетов для {0}",
                ["Cooldown"] = "Ты должен подождать {0} секунд, чтобы использовать эту команду снова.",
                ["AlreadyBanned"] = "{0} уже заблокирован!",
                ["NoPermission"] = "У вас нет разрешения на эту команду!",
                ["Disabled"] = "В настоящее время эта функция отключена.",
                ["Failed"] = "Не удалось отправить отчет, обратитесь к владельцу сервера.",
                ["ToSelf"] = "Вы не можете выполнить это действие на себе.",
                ["ReportTooShort"] = "Ваш отчет был слишком коротким! Пожалуйста, будьте более описательными.",
                ["PlayerChatFormatNew"] = "[{2}] **{0}:** {1}",
                ["BanPrefix"] = "Заблокирован/ы: {0}",
                ["Embed_ReportPlayer"] = "Репортер",
                ["Embed_ReportTarget"] = "Сообщено",
                ["Embed_ReportCount"] = "Количество сообщений",
                ["Embed_ReportReason"] = "Причина",
                ["Embed_Online"] = "онлайн",
                ["Embed_Offline"] = "Не в сети",
                ["Embed_ReportStatus"] = "Статус",
                ["Embed_ReportTitle"] = "Отчет об игроке",
                ["Embed_MuteTitle"] = "Игрок отключен",
                ["Embed_MuteTarget"] = "Игрок",
                ["Embed_MutePlayer"] = "Приглушено",
                ["Embed_BanPlayer"] = "Запрещено",
                ["Embed_BanTarget"] = "Игрок",
                ["Embed_BanReason"] = "Причина",
                ["Embed_BanTitle"] = "Бан игрока",
                ["Embed_MessageTitle"] = "Сообщение игрока",
                ["Embed_MessagePlayer"] = "Игрок",
                ["Embed_MessageMessage"] = "Сообщение",
                ["Embed_MuteTime"] = "Время",
                ["Embed_MuteReason"] = "Причина"
            }, this );
        }
        ...
        private const string BanPermission = "rust2discord.ban";
        private const string ReportPermission = "rust2discord.report";
        private const string MessagePermission = "rust2discord.message";
        private const string AdminPermission = "rust2discord.admin";
        ...
        private void API_SendFancyMessage( string webhookUrl, string content, string embedJsonString, Plugin plugin = null )
        {
            ...
            {
                PrintWarning( "Не удалось десериализовать строку json: {0}", embedJsonString );
                return;
            }
        ...
        private void API_SendFancyMessage( string webhookUrl, string embedName, int embedColor, string json, string content = null, Plugin plugin = null )
        {
            ...
            {
                PrintWarning( "Не удалось десериализовать строку json: {0}", json );
                return;
            }
        ...
        {
            if ( ( player.Name == "Консоль сервера" ) | !player.IsConnected )
            {
                return;
            }
